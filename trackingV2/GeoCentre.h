...
private:
	...
	void massCenters() {
		...
		// Utilisation d'un filtre de Canny
		Canny(foregroundMask, canny_output, 50, 150, 3);
		
		// Contours du filtre de Canny
		findContours(canny_output, contours, hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, Point(0, 0));
		
		// On récupère les "moments" de l'image
		vector<Moments> mu(contours.size());
		for (unsigned int i = 0; i < contours.size(); i++) {
			mu[i] = moments(contours[i], false);
		}
		
		// On récupère le centre de masse pour chaque contours
		vector<Point2f> mc(contours.size());
		for (unsigned int i = 0; i < contours.size(); i++) {
			// Récupération et affichage de chaque centre de masse
			mc[i] = Point2f(mu[i].m10 / mu[i].m00, mu[i].m01 / mu[i].m00);
			circle(foregroundMask, mc[i], 2, Scalar(128, 0, 0), 2);
		}

		m_result = foregroundMask;
		m_points = mc; // liste des centres de masse
		//cout << m_points << endl;
